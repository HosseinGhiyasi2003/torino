    ...
    return (
      <div className="container">
        <div className="mt-[23px] mb-12 w-full lg:border-1 border-[#00000033] lg:mb-[150px] lg:rounded-[10px] lg:p-[13px] lg:mt-4">
          <h3 className="text-center lg:text-2xl text-red-500">مشکلی در دریافت تورها به وجود اومده</h3>
        </div>
      </div>
    );
  }
    ...
    <div className="mt-[23px] mb-12 w-full lg:border-1 border-[#00000033] lg:mb-[150px] lg:rounded-[10px] lg:p-[13px] lg:mt-4">
      {data?.data?.length !== 0 ? (
        data?.data.map((tourDetail) => (
          <MyTourDetails key={tourDetail.id} tourDetail={tourDetail} isPending={isPending} />
        ))
      ) : (
